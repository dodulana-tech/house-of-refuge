      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div className="frow">
            <div className="fg"><label className="flabel">Patient *</label>
              <select className="fi" value={form.patient} onChange={e => setForm(p => ({ ...p, patient: e.target.value }))}>
                ...
                <option>Chidi Okonkwo</option><option>Adaeze Nnamdi</option><option>Kunle Adeyemi</option><option>Ibrahim Musa</option>
              </select>
            </div>
          ...
          <div className="fg"><label className="flabel">Offense Type *</label>
            <input className="fi" value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))} placeholder="e.g. Lateness to sessions, dormitory cleanliness" />
          </div>
          <div className="fg"><label className="flabel">Description</label>
            <textarea className="fi" rows={3} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Full details of the incident — what happened, when, who was involved..." />
          </div>
          <div className="fg"><label className="flabel">Response / Action Taken</label>
            <textarea className="fi" rows={2} value={form.response} onChange={e => setForm(p => ({ ...p, response: e.target.value }))} placeholder="Verbal warning, written reflection, privilege loss, etc." />
          </div>
