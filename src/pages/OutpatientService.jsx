...
import { getServiceBySlug, SERVICE_CATEGORIES } from '../utils/outpatient'
        ...
        <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 40, alignItems: 'start' }}>
          <div>
            ...
            {svc.conversion_eligible && (
              <div className="card" style={{ background: 'rgba(26,122,74,.06)', border: '1px solid rgba(26,122,74,.2)', borderLeft: '3px solid #1A7A4A', padding: '18px 22px', marginBottom: 20 }}>
                ...
                <p style={{ fontSize: '.95rem', lineHeight: 1.65, margin: 0 }}>
                  If your family proceeds to residential admission within <strong>{svc.conversion_window_days} days</strong> of this service, the fee is <strong>credited in full</strong> against the inpatient admission deposit.
                </p>
              </div>
            )}
            ...
            <ul style={{ paddingLeft: 22, lineHeight: 1.8, color: 'var(--charcoal)' }}>
              <li>You request the session online; our team confirms the slot and the fee with you by phone or email.</li>
          ...
          <aside style={{ position: 'sticky', top: 100, alignSelf: 'start' }}>
            <div className="card" style={{ padding: '24px 26px' }}>
              <div style={{ fontSize: '.72rem', letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 700, marginBottom: 6 }}>This session</div>
              <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--charcoal)', letterSpacing: '-.02em', lineHeight: 1.25 }}>
                {svc.duration_minutes ? `${svc.duration_minutes} minutes` : 'Package'}
              </div>
              <div style={{ fontSize: '.82rem', color: 'var(--g700)', marginTop: 4 }}>
                {cat?.label || svc.category} · HOR clinic, Lekki
              </div>
              ...
              <div style={{ fontSize: '.85rem', color: 'var(--charcoal)', lineHeight: 1.65, marginBottom: 18 }}>
                Send a request and our team confirms the slot, the practitioner, and the fee with you within one working day. Nothing is charged until you have the fee in writing and have agreed to it.
              </div>
